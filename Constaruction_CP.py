...
def get_valid_codes(max_length):
    """Генерирует все допустимые коды модулей P (Тайль)"""
...
def create_cp_folders():
    """Создает папки CP для допустимых комбинаций модулей Тайль"""
    ...
    print("Этап 1: Создание папок CP")
    print("-" * 40)
    ...
    print(f"Всего папок CP: {len(valid_codes)}")
    print()


# ====================== 2. Создание файлов с размерами ======================
def calculate_sizes(modules, module_lengths):
    """Вычисляет размеры дивана на основе последовательности модулей Тайль"""
    if len(modules) <= 1:
...
def create_size_files():
    """Создает файлы size.txt в папках CP для модулей Тайль"""
    ...
    # Фильтруем только модули Тайль (код начинается с P)
    teil_df = df[df["Код модуля"].str.startswith("P", na=False)].copy()

    # Создаем словарь с длинами модулей
    module_lengths = {}
    for _, row in teil_df.iterrows():
    ...
    try:
        ...
        cp_folders = [f for f in all_folders
    ...
    print(f"Найдено папок CP: {len(cp_folders)}")
...
def create_snake_image(modules, cells_folder):
    """Создает изображение змейкой из нескольких модулей Тайль"""
    images = []
    positions = []
    ...
    for i, module in enumerate(modules):
        module_filename = f"P{module}.png"
...
def create_cell_images():
    """Создает изображения cells.png в папках CP для модулей Тайль"""
    ...
    print(f"Обработка папок CP: {len(cp_folders)}")
    ...
    for folder in cp_folders:
        ...
        # Создаем изображение
        if len(modules) == 1:
            module_filename = f"P{modules[0]}.png"
            module_path = os.path.join(cells_folder, module_filename)
...
def main():
    print("=" * 50)
    print("ОБЪЕДИНЕННЫЙ СКРИПТ ДЛЯ МОДУЛЕЙ P (ТАЙЛЬ)")
    print("=" * 50)
